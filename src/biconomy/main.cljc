(ns biconomy.main
  "Kotodama WASM entrypoint for the Biconomy clean-room actor (L4) — Clojure port.
  ...
  facts (`biconomy.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "biconomy")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"biconomy.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "biconomy-compat" :tier tier :entities entities} 200])
...
(defn start! [] :biconomy-compat/ready)
